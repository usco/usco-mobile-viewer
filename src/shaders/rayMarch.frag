...
#pragma glslify: sdPlane = require('./primitives/plane.frag')
...
vec2 map( in vec3 pos )
{
		...
		//vec2 res = vec2( sdBox( pos-vec3( 1.0,0.25, 0.0), vec3(0.25) ), 110.0 );
    //vec2 res = opU( vec2( sdPlane(pos), 1.0 ),
	  //              vec2( sdSphere(pos-vec3( 0.0, 0.5, 0.0), 0.9 ), 26.9 ) );
		float sphere1R = 0.9;
		float sphere2R = 0.8;
		float sphere3R = 0.5;
		float sphere4R = 0.2;

		float sphere1 = sdSphere(pos-vec3( 0.0, 0.5, 0.0), sphere1R );
  ...
  res = opU( res, vec2( sdCapsule(   pos,vec3(-1.3,0.10,-0.1), vec3(-0.8,0.50,0.2), 0.1  ), 31.9 ) );
	...
	// various geometric operations applied to the primitives
